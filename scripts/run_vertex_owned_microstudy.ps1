...
param(
    [string]$ProjectRoot = '',
    [string]$RunLabel = 'paper-20260724-vertex-owned-r1',
    [string]$RunRoot = '',
...
if ($TimingFrames -lt 1 -or $TimingWarmup -lt 0 -or $TimingRepetitions -ne 3 -or $QualityFrames -lt 1 -or $QualityWarmup -lt 0) {
    throw 'Formal vertex-owned microstudy requires positive frames, nonnegative warm-up, and exactly three timing repetitions.'
}

$benchmarkScript = Join-Path $scriptDir 'run_benchmark.ps1'
$analysisScript = Join-Path $scriptDir 'analyze_vertex_owned_microstudy.py'
$budgetPath = Join-Path $CalibrationRoot 'selected_budgets.csv'
foreach ($path in @($benchmarkScript, $analysisScript, $budgetPath)) {
    if (-not (Test-Path -LiteralPath $path)) { throw "Missing vertex-owned microstudy input: $path" }
}
...
$manifest = [ordered]@{
    protocol_version = 'vertex-owned-microstudy-v1'
    label = $RunLabel
    git_commit = $commit
    ...
    structural_metrics = [ordered]@{
        edge_scatter_scalar_atomic_writes = '6 * spring_constraint_count + 3 * attachment_constraint_count'
        gradient_related_barriers = 'gradient_dispatches + stats_dispatches'
        full_vector_gradient_passes = [ordered]@{ 'gpu-edge-scatter' = 2; 'gpu-gather-no-fusion' = 2; 'gpu-gather-fusion' = 1 }
    }
}
[System.IO.File]::WriteAllText((Join-Path $RunRoot 'manifest.json'), ($manifest | ConvertTo-Json -Depth 9), [System.Text.UTF8Encoding]::new($false))
$cases | Export-Csv -LiteralPath (Join-Path $RunRoot 'planned_cases.csv') -NoTypeInformation
if ($DryRun) { Write-Host "Vertex-owned microstudy dry-run: $RunRoot"; exit 0 }

foreach ($case in $cases) {
    $caseId = "$($case.scene_id)-d$($case.cloth_dimension)"
    foreach ($variant in $variants) {
        for ($rep = 1; $rep -le $TimingRepetitions; ++$rep) {
            $outputDir = Join-Path $RunRoot (Join-Path 'timing' (Join-Path $caseId (Join-Path $variant ('rep{0:D2}' -f $rep))))
            if ((Test-Path -LiteralPath (Join-Path $outputDir 'frame_presentation.csv')) -and -not $Force) { continue }
            $params = @{
                ProjectRoot = $ProjectRoot; RunLabel = "$RunLabel-$caseId-$variant-rep$('{0:D2}' -f $rep)"; OutputDir = $outputDir
                SolverVariant = $variant; IterationsPerFrame = $case.iterations_per_frame
        ...
        $qualityDir = Join-Path $RunRoot (Join-Path 'quality' (Join-Path $caseId $variant))
        if (-not (Test-Path -LiteralPath (Join-Path $qualityDir 'quality_metrics.csv')) -or $Force) {
            $qualityParams = @{
                ProjectRoot = $ProjectRoot; RunLabel = "$RunLabel-quality-$caseId-$variant"; OutputDir = $qualityDir
                SolverVariant = $variant; IterationsPerFrame = $case.iterations_per_frame
...
& $PythonExe $analysisScript --run-root $RunRoot --report-path (Join-Path $ProjectRoot 'docs\experiments\2026-07-24-vertex-owned-microstudy.md')
if ($LASTEXITCODE -ne 0) { throw "Vertex-owned microstudy analysis failed with exit code $LASTEXITCODE." }
Write-Host "Vertex-owned microstudy complete: $RunRoot"
